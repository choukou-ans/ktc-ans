	 ...
	 public function list_data_bml() {
		...
		$data_finds = $this->Spec->find('all', array(
			...
			'fields' => array('id', 'order_num', 'delivery_date', 'num', 'unit', 'block_copy_total', 'plate_makeing_total', 'shipping_total', 
				...
				'm_reprint_id', 'unit_price', 'packaging', 'delivery_memo', 'price'
			),
			'recursive' => -1,
		...
		foreach ($data_finds as $index => &$data_find) {
			...
			//get  delivery_notes.delivery_date
			$delivery_date = $this->DeliveryNoteDetail->find('first', array(
				'conditions' => array(
					'DeliveryNoteDetail.spec_id' => $data_find['Spec']['id'],
				),
				'fields' => array('DeliveryNote.delivery_date'),
				'recursive' => 0,
				'order' => array('DeliveryNoteDetail.id DESC'),
			));
			
			if(empty($delivery_date['DeliveryNote']['delivery_date'])){
				$data_find['DeliveryNote']['delivery_date'] = $data_find['Spec']['delivery_date'];
			}else{
				$data_find['DeliveryNote']['delivery_date'] = $delivery_date['DeliveryNote']['delivery_date'];
			}
        }
